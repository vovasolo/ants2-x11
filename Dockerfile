...
RUN tar -xzf v$VERSION.tar.gz -C /opt
...
RUN cd /geant4-build && cmake -DCMAKE_INSTALL_PREFIX=/opt/geant4-install -DGEANT4_USE_GDML=ON -DCMAKE_BUILD_TYPE=Release -DGEANT4_INSTALL_DATA=ON -DGEANT4_USE_OPENGL_X11=ON -DGEANT4_USE_XM=ON -DGEANT4_USE_QT=ON -DGEANT4_BUILD_MULTITHREADED=OFF /opt/geant4-$VERSION && make -j$JOBS && make install
